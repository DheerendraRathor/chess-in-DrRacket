...
(define (update-move-text str clr)
  (if (string=? clr "white")
      (send move-text set-value (string-append (send move-text get-value) "\n " (number->string counter) ". "  str))
      (begin (send move-text set-value (string-append (send move-text get-value) " - " str)) (set! counter (add1 counter)))))
...
(define (getval w m)
  (cond [(eq? m 'mate) (begin (set! dropped #t) w)]
        ...
        [else (let ([posi (apply make-posn (car m))]
                    [posf (apply make-posn (cadr m))]
                    [time (caddr m)])
                (begin (update-move-text (move-write posi posf) (if (string=? color "white") "black" "white")) 
                       
                       (send (get posi board-conf) move posf) (set! opptime time)  
                       (if (string=? color "white") (update-valid-moves-check-w) (update-valid-moves-check-b))
                    ...
                    (if (eq? calln #f) (let* ([pos (cord-to-pos x y)]
                        ...
                        (let* ([pos (cord-to-pos x y)]
                          ...
                          (cond [(or (number? checking) (not (string=? (get-field color checking) color)))
                                 (if (member pos moves)
                                     (begin (update-move-text (move-write cordie pos) color) 
                                            (play-sound move-sound #t) (send peace move pos) (set! calln #f) 
